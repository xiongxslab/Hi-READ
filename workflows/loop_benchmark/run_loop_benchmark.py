#!/usr/bin/env python3
"""
Run peak-to-peak FitHiChIP loop calling on NTKO/TKO chr15 npy files across four methods
(real, corigami, hicdiffusion, RRTdiffusion) and compare prediction performance against real data.

Conventions:
- All tasks process only chr15 npy files
- NTKO/TKO use their respective bed.gz peak files
...
DEFAULT_EXPERIMENT_ROOT = str(
    Path(
        os.environ.get(
            ...
            RELEASE_ROOT / "release_assets" / "loop_benchmark" / "peak2peak_ntko_tko_chr15_20260325",
        )
    )
)
...
PEAK_FILES = {
    "NTKO": str(Path(MACS2_OUTPUT_DIR) / "NTKO" / "GSM3791748_4-NTKO2-1_CTCF_noinput_peaks.bed.gz"),
    "TKO": str(Path(MACS2_OUTPUT_DIR) / "TKO" / "GSM3791750_6-TKO1-1_CTCF_noinput_peaks.bed.gz"),
}
...
METHOD_CONFIGS = {
    "real": {
        "datasets": {
            "NTKO": {
                "source_npy_dir": str(REAL_ROOT / "NTKO" / "chr15" / "CTCF" / "Real" / "npy"),
                "subset_mode": "all_chr15",
            },
            "TKO": {
                "source_npy_dir": str(REAL_ROOT / "TKO" / "chr15" / "CTCF" / "Real" / "npy"),
                "subset_mode": "all_chr15",
            },
        },
    },
    "corigami": {
        "datasets": {
            "NTKO": {
                "source_npy_dir": str(CORIGAMI_ROOT / "NTKO"),
                "subset_mode": "all_chr15",
            },
            "TKO": {
                "source_npy_dir": str(CORIGAMI_ROOT / "TKO"),
                "subset_mode": "all_chr15",
            },
        },
    },
    "hicdiffusion": {
        "datasets": {
            "NTKO": {
                "source_npy_dir": str(HICDIFFUSION_ROOT / "NTKO"),
                "subset_mode": "all_chr15",
            },
            "TKO": {
                "source_npy_dir": str(HICDIFFUSION_ROOT / "TKO"),
                "subset_mode": "all_chr15",
            },
        },
    },
    "RRTdiffusion": {
        "datasets": {
            "NTKO": {
                "source_npy_dir": str(RRTDIFFUSION_ROOT / "NTKO"),
                "subset_mode": "final_only",
            },
            "TKO": {
                "source_npy_dir": str(RRTDIFFUSION_ROOT / "TKO"),
                "subset_mode": "final_only",
            },
        },
    },
...
class NTKOTKOChr15MultiMethodPeak2Peak:
    ...
    def _prepare_subset(self, method_name, dataset_name, source_dir, subset_mode):
        ...
        if linked == 0:
            raise RuntimeError(
                f"{method_name}/{dataset_name} pattern not found {pattern}: {source_dir}"
            )
    ...
    def _run_single_task(self, method_name, dataset_name, config):
        ...
        peaks_file = PEAK_FILES[dataset_name]
        cmd = [
            FIT_HICHIP_PYTHON,
            CALLER_SCRIPT,
            "--npy-dir", subset_dir,
        ...
        with open(log_file, "w") as log_f:
            log_f.write(f"Start time: {start_time.isoformat()}\n")
            ...
            log_f.write(f"peaks_file: {peaks_file}\n")
    ...
    def _run_all_loops(self):
        ...
        print("=" * 80)
        print("NTKO/TKO chr15 multi-method peak-to-peak FitHiChIP starting")
        print("=" * 80)
        print(f"Output directory: {self.loops_root}")
        print(f"Parallel jobs: {self.parallel_jobs}")
        print(f"NPY workers per task: {self.npy_workers}")
        ...
        with ThreadPoolExecutor(max_workers=self.parallel_jobs) as executor:
            ...
            for future in as_completed(future_to_task):
                ...
                print(
                    ...
                    f"({result.get('duration_minutes', 0)} minutes)"
                )
    ...
    def _run_single_comparison(self, method_name):
        ...
        for dataset_name in ["NTKO", "TKO"]:
            gt_link = gt_dir / f"{dataset_name}_loops.bed"
            pred_link = pred_dir / f"{dataset_name}_loops.bed"
            ...
            if not gt_target.exists() or not pred_target.exists():
                raise RuntimeError(
                    f"comparison missing loops file: {method_name}/{dataset_name} "
    ...
    def run(self):
        summary = {
            ...
            "parameters": {
                ...
                "force": self.force,
                "note": "NTKO/TKO chr15 only; all methods use peak-to-peak with explicit bed.gz peaks; RRTdiffusion uses final.npy only",
            },
            "loop_call_results": [],
            "comparison_results": [],
        }
...
def main():
    parser = argparse.ArgumentParser(
        description="NTKO/TKO chr15 multi-method peak-to-peak FitHiChIP + comparison"
    )
    parser.add_argument(
        "--experiment-root",
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of parallel npy->bed processes per task",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    runner = NTKOTKOChr15MultiMethodPeak2Peak(
