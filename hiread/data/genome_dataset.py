...
from hiread.data.chromosome_dataset import ChromosomeDataset
import hiread.data.data_feature as data_feature
...
class GenomeDataset(Dataset):
    ...
    def __init__(self, celltype_root, 
        ...
        self.val_chromosomes = list(val_chromosomes or default_val)
        self.test_chromosomes = list(test_chromosomes or default_test)

        # Assign train/val/test chromosomes
        self.chr_names = self.get_chr_names(genome_assembly)
        if mode == 'train':
            if self.train_chromosomes is not None:
                self.chr_names = list(self.train_chromosomes)
            else:
                excluded = set(self.val_chromosomes + self.test_chromosomes)
                if omit_chr_x:
                    excluded.add('chrX')
                self.chr_names = [name for name in self.chr_names if name not in excluded]
        elif mode == 'val':
            self.chr_names = self.val_chromosomes
        elif mode == 'test':
            self.chr_names = self.test_chromosomes
        else:
        ...
        self.centrotelo_dict = self.proc_centrotelo(centrotelo_path or f'{celltype_root}/../centrotelo.bed')
    ...
    def load_chrs(self, chr_names, genomic_features):
        ...
        for chr_name in chr_names:
            omit_regions = self.centrotelo_dict.get(chr_name)
            if omit_regions is None:
                omit_regions = pd.DataFrame(columns=['start', 'end']).to_numpy().reshape(0, 2)
    ...
    def proc_centrotelo(self, bed_dir):
        ...
        if not os.path.exists(bed_dir):
            raise FileNotFoundError(f'centrotelo bed file not found: {bed_dir}')
...
if __name__ == '__main__':
    raise SystemExit("Import this module from training code.")
