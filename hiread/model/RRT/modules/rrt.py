...
class RRTEncoder(nn.Module):
    ...
    def forward(self, x, need_attn=False):
        ...
        # 存储attention权重
        attn_weights = [] if need_attn else None
        ...
        for i,layer in enumerate(self.layers.children()):
            ...
            if need_attn:
                x, attn = layer(x, need_attn=True)
                attn_weights.append(attn)
            else:
                x = layer(x)

        # CR-MSA
        if need_attn and hasattr(self.cr_msa, 'forward'):
            x, cr_attn = self.cr_msa(x, need_attn=True)
            attn_weights.append(cr_attn)
        else:
            x = self.cr_msa(x)
